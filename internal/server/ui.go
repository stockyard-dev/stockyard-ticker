...
async function load(){var r=await fetch(A+'/prices').then(function(r){return r.json()});items=r.prices||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.exchange||'').toLowerCase().includes(q)||(i.price||'').toLowerCase().includes(q)||(i.change_pct||'').toLowerCase().includes(q)});
...
if(i.exchange)h+='<div class="item-sub">'+esc(i.exchange)+'</div>';
...
if(i.price)h+='<span>'+esc(i.price)+'</span>';
if(i.change_pct)h+='<span>'+esc(i.change_pct)+'</span>';
if(i.volume)h+='<span>'+esc(i.volume)+'</span>';
if(i.high)h+='<span>'+esc(i.high)+'</span>';
if(i.low)h+='<span>'+esc(i.low)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/prices/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",exchange:"",price:"",change_pct:"",volume:"",high:"",low:"",status:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' TICKER</h2>';
h+='<div class="fr"><label>Symbol *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Exchange</label><input id="f-exchange" value="'+esc(i.exchange)+'"></div>';
h+='<div class="fr"><label>Price</label><input id="f-price" value="'+esc(i.price)+'"></div>';
h+='<div class="fr"><label>Change</label><input id="f-change-pct" value="'+esc(i.change_pct)+'"></div>';
h+='<div class="fr"><label>Volume</label><input id="f-volume" value="'+esc(i.volume)+'"></div>';
h+='<div class="fr"><label>High</label><input id="f-high" value="'+esc(i.high)+'"></div>';
h+='<div class="fr"><label>Low</label><input id="f-low" value="'+esc(i.low)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),exchange:document.getElementById('f-exchange').value.trim(),price:document.getElementById('f-price').value.trim(),change_pct:document.getElementById('f-change-pct').value.trim(),volume:document.getElementById('f-volume').value.trim(),high:document.getElementById('f-high').value.trim(),low:document.getElementById('f-low').value.trim(),status:document.getElementById('f-status').value.trim()};
if(editId){await fetch(A+'/prices/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/prices',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
