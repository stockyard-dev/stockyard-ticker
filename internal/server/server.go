...
s.mux.HandleFunc("PUT /api/prices/{id}",s.update)
s.mux.HandleFunc("DELETE /api/prices/{id}",s.del)
s.mux.HandleFunc("GET /api/stats",s.stats)
s.mux.HandleFunc("GET /api/health",s.health)
s.mux.HandleFunc("GET /ui",s.dashboard);s.mux.HandleFunc("GET /ui/",s.dashboard);s.mux.HandleFunc("GET /",s.root);
s.mux.HandleFunc("GET /api/tier",func(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"tier":s.limits.Tier,"upgrade_url":"https://stockyard.dev/ticker/"})})
return s}
...
func(s *Server)create(w http.ResponseWriter,r *http.Request){if s.limits.MaxItems>0{if s.db.Count()>=s.limits.MaxItems{we(w,402,"Free tier limit reached. Upgrade at https://stockyard.dev/ticker/");return}};var e store.Price;json.NewDecoder(r.Body).Decode(&e);if e.Symbol==""{we(w,400,"name required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
...
func(s *Server)update(w http.ResponseWriter,r *http.Request){
    ...
    var patch store.Price;json.NewDecoder(r.Body).Decode(&patch);patch.ID=existing.ID;patch.CreatedAt=existing.CreatedAt
    if patch.Symbol==""{
